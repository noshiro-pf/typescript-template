...
That filter is the scope rule, and a draft PR cannot have auto-merge, so it does
most of the exclusion by itself. Still drop any PR whose `baseRefName` is not
...
page shows. `test (test:cov)`, `test-typescript-versions (*)`, `codecov/*` and
...
**The check name is the command.** `style-check (X)`, `lint-and-build (X)` and
`test (X)` all run `pnpm run X`, so `lint-and-build (codemod:full)` reproduces as
`pnpm run codemod:full`. Four checks do not follow that rule:

| Check                              | What to run                                                                                    |
| :--------------------------------- | :--------------------------------------------------------------------------------------------- |
| `test-node-versions (<ver>)`       | `pnpm run build` then `pnpm run test` on that Node                                             |
| `test-typescript-versions (<ver>)` | `pnpm run build:min`, then `npx -y -p typescript@<ver> tsc -p ./test/dist/named/tsconfig.json` |
...
- **`test (*)` runs `pnpm run build:min` first in CI.** Do the same locally, or
  the tests run against a stale `dist/` and fail for the wrong reason.
  `style-check (doc)` and `style-check (check:ext)` do not build, so they read
  whatever `dist/` you already have.
- **`test (test:browser)` needs Playwright** — `pnpm exec playwright install
chromium` — and CI retries it once before calling it failed, so a single red
  run there may just be flake. Re-read the log before changing code.
- **Every `style-check`, `lint-and-build` and `test` job ends with
  `assert-repo-is-clean`.** So `fmt:full`, `doc`, `codemod:full` and `lint:fix`
  fail by _changing_ files, and the fix is to run the command locally and commit
  what it wrote. A green run of the command with a dirty tree afterwards is still
  a failure.
...
no `as any`. Remember that `AGENTS.md` itself is generated — change
`agents/local-rules.md` and run `pnpm run agents:gen`. Then `pnpm run fmt`,
...
`lint-and-build`, `test` and the two compatibility matrices all gate on
